<?php
/* -----------------------------------------------------------------------------------------
   $Id: xss_secure.php 16743 2026-01-08 13:04:36Z GTB $
...
if (defined('XSS_BLACKLIST') && XSS_BLACKLIST) {
  ...
  // Whitelist-Spiegel prüfen (dateibasiert, keine DB nötig - siehe bx_sync_whitelist_file)
  $is_whitelisted = false;
  $whitelist_file = XSS_PATH . 'log/msec_whitelist_mirror.log';
  if (is_file($whitelist_file)) {
      $whitelisted_ips = @file($whitelist_file, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
      if ($whitelisted_ips !== false) {
          $is_whitelisted = in_array($ip, $whitelisted_ips, true);
      }
  }
